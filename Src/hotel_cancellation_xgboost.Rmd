...
print(paste0('Number of training data set: ', nrow(df_train)))
print(paste0('Number of testing data set: ', nrow(df_test)))
```
...
```{r}
bst <- xgboost(data = train, label = IsCanceled_train, max.depth = 2, eta = 1, nthread = 2, nrounds = 600, objective = "binary:logistic")
summary(bst)
```
...
#Missclassification Rate for test dataset
```{r}
...
```{r}
confusionMatrix(as.factor(prediction), as.factor(IsCanceled_test), positive="1")
```